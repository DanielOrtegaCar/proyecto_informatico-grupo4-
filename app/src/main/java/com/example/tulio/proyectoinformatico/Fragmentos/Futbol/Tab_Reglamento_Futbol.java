...
public class Tab_Reglamento_Futbol extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle bundle) {
        View rootView = inflater.inflate(R.layout.reglamentos, container, false);

        mRecyclerView = rootView.findViewById(R.id.lista_reglamento); // cambiar id reciccler view R.id.lista
        mRecyclerView.setHasFixedSize(true);
        mLayoutManager = new LinearLayoutManager(this.getContext());
        mRecyclerView.setLayoutManager(mLayoutManager);

        RestClient client = RetrofitUtils.getInstance().create(RestClient.class);
        Call<List<Reglamento>> call = client.getReglamento(1); // cambiar List<Equipos> acomoodar con modelo que quiero llamar
...
class AdapterReglamento extends RecyclerView.Adapter<AdapterReglamento.ElementoTabla> {
    ...
    @Override
    // crea la vista
    public AdapterReglamento.ElementoTabla onCreateViewHolder(ViewGroup parent,
                                                                                                                    int viewType) {
        // create a new view
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.futbol_reglamento, parent, false);
    ...
    @Override
    public void onBindViewHolder(@NonNull ElementoTabla holder, int posición) {
